...
//                         my_rank + 1, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
            ...
            MPI_Isend(&cur[i * colEnd + (colEnd - 1)], 1, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD, &request);
            MPI_Irecv(&tempRow[i], 1, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD, &request);

        }
    } else if (my_rank == comm_sz - 1) {
        for (int i = 0; i < n; ++i) {
            MPI_Isend(&cur[i * colEnd + 0], 1, MPI_DOUBLE, my_rank - 1, 0, MPI_COMM_WORLD, &request);
            MPI_Irecv(&tempRow[i], 1, MPI_DOUBLE, my_rank - 1, 0, MPI_COMM_WORLD, &request);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            MPI_Isend(&cur[i * colEnd + (colEnd - 1)], 1, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD, &request);
            MPI_Irecv(&tempRowBelow[i], 1, MPI_DOUBLE, my_rank + 1, 0, MPI_COMM_WORLD, &request);
